...
interface BulkUnsuspendAccountProps {
	ids: number[];
	onUnsuspended?: () => void | Promise<void>;
}

export function BulkUnsuspendAccount(props: BulkUnsuspendAccountProps) {
				...
				<Dialog.Positioner>
					<Dialog.Content className="min-w-120 max-w-120">
						<Dialog.Header>
							...
							<div>
								<Dialog.Title>Unsuspend admins</Dialog.Title>
								<Dialog.Description>
								...
								onClick={async () => {
									try {
										...
										toaster.success({
											description:
												props.ids.length > 1
													? 'Admins have been unsuspended'
													: 'Admin has been unsuspended',
										});
									} catch (error) {
										toaster.error({
