...
import {useExportExpenseMutation} from '~/hooks/useExportExpenseMutation';
import {useFilterContext} from './ExpensesContext';

export function ExportExpenses() {
	const filter = useFilterContext();

	const exportExpenseMutation = useExportExpenseMutation();
		...
		>
			<Tooltip.Root>
				<Tooltip.Trigger asChild>
					<IconButton
						disabled={(expenseAggregateQuery.data?.total ?? 0) <= 0}
						onClick={() => {
					...
					<Dialog.Content className="min-w-116 max-w-116" asChild>
						<form
							...
							onSubmit={form.handleSubmit(async ({fileName}) => {
								try {
									const blob = await exportExpenseMutation.mutateAsync(filter);
						...
						>
							<Dialog.Header>
								...
								<div>
									<Dialog.Title>Export expense</Dialog.Title>
							...
							<Dialog.Footer>
								<Button
									...
									disabled={exportExpenseMutation.isPending}
								>
									Cancel
								</Button>
								<Button type="submit" disabled={exportExpenseMutation.isPending}>
									Export
								</Button>
							</Dialog.Footer>
						</form>
