...
interface BulkUnsuspendMemberProps {
	ids: number[];
	onUnsuspended?: () => void | Promise<void>;
}

export function BulkUnsuspendMember(props: BulkUnsuspendMemberProps) {
				...
				<Dialog.Positioner>
					<Dialog.Content className="min-w-120 max-w-120">
						<Dialog.Header>
							...
							<div>
								<Dialog.Title>Unsuspend members</Dialog.Title>
								<Dialog.Description>
							...
							<Button
								...
								onClick={async () => {
									try {
										...
										await props.onUnsuspended?.();

										disclosure.setOpen(false);
										toaster.success({
											description:
												props.ids.length > 1
													? 'Members have been unsuspended'
													: 'Member has been unsuspended',
										});
									} catch (error) {
										toaster.error({
