...
export const HeadingTrigger = forwardRef<HTMLButtonElement, WysiwygHeadingTriggerProps>(
	(props, ref) => {
		const wysiwyg = useWysiwygContext();

		const [headingTriggerProps, localProps] = useMemo(() => splitProps(props, 'level'), [props]);

		const mergedProps = mergeProps(wysiwyg.getHeadingTriggerProps(headingTriggerProps), localProps);
